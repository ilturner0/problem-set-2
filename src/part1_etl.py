...
def etl():
    '''
    Extracts the two datasets from the dropbox link, converts the date columns to the datetime data type, drops unnecessary columns, and loads the resulting dataframes to /data.
    ...
    pred_universe_raw.to_csv(r'data/pred_universe_raw.csv')
    arrest_events_raw.to_csv(r'data/arrest_events_raw.csv')